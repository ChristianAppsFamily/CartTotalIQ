...
export function getTaxRateForZip(zip: string): number {
  const prefix3 = zip.substring(0, 3);
  const state = ZIP_PREFIX_TO_STATE[prefix3] || ZIP_TO_STATE[zip.charAt(0)];
...
export function getStateForZip(zip: string): string {
  const prefix3 = zip.substring(0, 3);
  return ZIP_PREFIX_TO_STATE[prefix3] || ZIP_TO_STATE[zip.charAt(0)] || 'Unknown';
}
